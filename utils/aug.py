...
#df = pd.read_csv('pose2_record.csv')
...
class DataAugmenter: 
    ...
    def flip(self): 
        ...
        self.df.loc[mask, 'x'] = 1 -self.df.loc[mask, 'x']
    ...
    def add_noise(self, gaussian = False, noise_factor = 0.15):
        if gaussian: 
            sigma = 0.1 
            self.coords += np.random.normal(0, sigma, self.coords.shape)

        else: 
            noise = np.random.randn(*self.coords.shape) * noise_factor
            self.coords += noise 
...
aug = DataAugmenter('pose2_record.csv')
aug.flip()
aug.add_noise()
aug.random_resize()
aug.spatial_affine()
aug.center_crop()
aug.save('pose_aug.csv')
